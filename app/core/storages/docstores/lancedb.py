...
class LanceDBDocumentStore(BaseDocumentStore):
        ...
        **kwargs,
        ...
        if self.collection_name not in self.db_connection.table_names():
            if data:
                ...
                # Check if the id field exists and create index for it
                if "id" in data[0]:
            ...
            document_collection = self.open_table()
            if data:
    ...
    def query(
        self, query: str, top_k: int = 20, doc_ids: Optional[list] = None
    ) -> List[Document]:
        if doc_ids:
    ...
    def open_table(self):
        """Open the table store document"""
        return self.db_connection.open_table(self.collection_name)
